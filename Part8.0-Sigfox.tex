...
Le programme \lprog{sigfox\_id.py} permet d’afficher ces deux valeurs et d’envoyer un message sur le réseau Sigfox. 
...
\item remplissez le formulaire avec l'identifiant de l'objet et le \Index{PAC} que vous avez obtenus avec le programme \lprog{sigfox\_id.py} ;
...
Le mieux étant de remplir un fichier de configuration avec ces valeurs comme le montre le programme \lprog{config\_sigfox.py}.
...
Le programme \lprog{sigfox\_temperature.py} est une adaptation de \lprog{wifi\_temperature.py}, listing~\vref{prog-wifi-temp} dédié au Wi-Fi pour des transmission sur le réseau Sigfox.
...
\end{itemize}
 ...
 \pythonnxt[firstline=195,lastline=217, firstnumber=195]{generic\_relay.py}
